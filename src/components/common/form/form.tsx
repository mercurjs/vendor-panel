import React, { createContext, forwardRef, ReactNode, useContext, useId } from 'react';

import { CircleInfoSolid } from '@medusajs/icons';
import { clx, Hint as HintComponent, Label as LabelComponent, Text, Tooltip } from '@medusajs/ui';
...
>(({ ...props }, ref) => {
  ...
  return (
    <Slot.Root
      ...
      id={formItemId}
      aria-describedby={
        !error ? `${formDescriptionId}` : `${formDescriptionId} ${formErrorMessageId}`
      }
      aria-invalid={!!error}
...
const Hint = forwardRef<HTMLSpanElement, React.HTMLAttributes<HTMLSpanElement>>(
  ({ className, ...props }, ref) => {
...
const ErrorMessage = forwardRef<HTMLSpanElement, React.HTMLAttributes<HTMLSpanElement>>(
