...
export const createDataGridPriceColumns = <TData, TFieldValues extends FieldValues>({
  currencies,
  pricePreferences,
  isReadyOnly,
  getFieldName,
...
}: CreateDataGridPriceColumnsProps<TData, TFieldValues>): ColumnDef<TData, unknown>[] => {
    ...
    ...(currencies?.flatMap(currency => {
      ...
      const currentPriceCol = showCurrentPriceCell
        ? columnHelper.column({
            ...
            cell: context => {
              const prices = (context.row.original as HttpTypes.AdminProductVariant).prices;

              const currentPrice = prices?.find(({ currency_code }) => currency_code === currency);
      ...
      return [editableCol, currentPriceCol].filter(Boolean) as ColumnDef<TData, unknown>[];
    }) ?? [])
  ];
};
