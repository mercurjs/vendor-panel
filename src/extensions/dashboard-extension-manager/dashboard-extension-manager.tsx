...
export class DashboardExtensionManager {
  ...
  private populateMenus(menuItems: MenuItemExtension[] | undefined) {
    ...
    menuItems.forEach(item => {
      ...
      const parentItem = menuItems.find(menuItem => menuItem.path === parentPath);
  ...
  private populateForm(formModule: FormModule): {
    ...
    Object.entries(formModule.customFields).forEach(([model, customization]) => {
      fields.set(model as CustomFieldModel, this.processFields(customization.forms));
      configs.set(model as CustomFieldModel, this.processConfigs(customization.configs));
    });
  ...
  private populateDisplays(displayModule: DisplayModule): DisplayMap {
    ...
    Object.entries(displayModule.displays).forEach(([model, customization]) => {
      displays.set(model as CustomFieldModel, this.processDisplays(customization));
    });
