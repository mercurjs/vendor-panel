...
export const AddCampaignPromotionsForm = ({ campaign }: AddCampaignPromotionsFormProps) => {
  ...
  const { setValue } = form;
  const { mutateAsync, isPending } = useAddOrRemoveCampaignPromotions(campaign.id);
        ...
        <RouteFocusModal.Header>
          <div className="flex items-center justify-end gap-x-2">
            {form.formState.errors.promotion_ids && (
              <Hint variant="error">{form.formState.errors.promotion_ids.message}</Hint>
            )}
          </div>
        </RouteFocusModal.Header>
  ...
  return useMemo(
    () => [
      columnHelper.display({
        ...
        cell: ({ row, table }) => {
          const { campaignId, currencyCode, budgetType } = table.options.meta as {
            campaignId: string;
            currencyCode: string;
            budgetType: string;
          };
          ...
          const isAddedToADiffCampaign =
            !!row.original.campaign_id && row.original.campaign_id !== campaignId;
          const currencyMismatch =
            isTypeSpend && row.original.application_method?.currency_code !== currencyCode;
