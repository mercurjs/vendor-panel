...
import { Badge, Container, Heading, StatusBadge, Text, toast, usePrompt } from '@medusajs/ui';
...
import { campaignStatus, statusColor } from '../../../common/utils/campaign-status';

type CampaignGeneralSectionProps = {
  campaign: AdminCampaignResponse['campaign'];
};

export const CampaignGeneralSection = ({ campaign }: CampaignGeneralSectionProps) => {
    ...
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-x-4">
          <StatusBadge color={statusColor(status)}>{t(`campaigns.status.${status}`)}</StatusBadge>
