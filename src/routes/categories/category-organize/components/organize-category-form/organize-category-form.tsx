...
import { categoriesQueryKeys, useProductCategories } from '../../../../../hooks/api/categories';
...
export const OrganizeCategoryForm = () => {
  const { product_categories, isPending, isError, error: fetchError } = useProductCategories(QUERY);
  ...
  const { mutateAsync, isPending: isMutating } = useMutation({
    ...
    onMutate: async update => {
      ...
      const previousValue: HttpTypes.AdminProductCategoryListResponse | undefined =
        queryClient.getQueryData(categoriesQueryKeys.list(QUERY));
    ...
    onError: (error: FetchError, _newValue, context) => {
      ...
      queryClient.setQueryData(categoriesQueryKeys.list(QUERY), context?.previousValue);
