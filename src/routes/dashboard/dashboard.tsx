...
export const Dashboard = () => {
  ...
  const notFulfilledOrders =
    orders?.filter(order => order.fulfillment_status === 'not_fulfilled').length || 0;
  const fulfilledOrders =
    orders?.filter(order => order.fulfillment_status === 'fulfilled').length || 0;
  const reviewsToReply = reviews?.filter((review: any) => !review?.seller_note).length || 0;
