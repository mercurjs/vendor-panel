...
export const AdjustInventoryDrawer = () => {
  ...
  const inventoryLevel = location_levels?.find(
    (level: InventoryTypes.InventoryLevelDTO) => level.location_id === location_id
  );

  const { stock_location, isLoading: isLoadingLocation } = useStockLocation(location_id!);

  const ready =
    !isLoading && inventoryItem && inventoryLevel && !isLoadingLocation && stock_location;
