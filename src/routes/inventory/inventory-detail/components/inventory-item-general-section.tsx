...
export const InventoryItemGeneralSection = ({
  ...
  const stockedQuantity =
    inventoryItem.location_levels?.reduce((acc, level) => acc + level.stocked_quantity, 0) || 0;
  const reservedQuantity =
    inventoryItem.location_levels?.reduce((acc, level) => acc + level.reserved_quantity, 0) || 0;
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <SectionRow
        ...
        value={getQuantityFormat(stockedQuantity, inventoryItem.location_levels?.length)}
      />

      <SectionRow
        title={t('inventory.reserved')}
        value={getQuantityFormat(reservedQuantity, inventoryItem.location_levels?.length)}
      />
      <SectionRow
        title={t('inventory.available')}
        value={getQuantityFormat(availableQuantity, inventoryItem.location_levels?.length)}
      />
    </Container>
  );
};
