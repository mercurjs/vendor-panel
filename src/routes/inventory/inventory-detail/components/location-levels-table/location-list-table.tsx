...
export const ItemLocationListTable = ({ inventory_item_id }: { inventory_item_id: string }) => {
  ...
  const { location_levels, count, isLoading } = useInventoryItemLevels(inventory_item_id, {
    ...searchParams,
    fields: '*stock_locations'
  });
  const columns = useLocationListTableColumns();

  const filteredLocationLevels = location_levels?.filter(level => level.stock_locations.length > 0);
