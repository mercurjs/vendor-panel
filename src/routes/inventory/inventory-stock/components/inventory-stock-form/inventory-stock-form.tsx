...
import { InventoryItemSchema, InventoryLocationsSchema, InventoryStockSchema } from '../../schema';
...
export const InventoryStockForm = ({ items, locations }: InventoryStockFormProps) => {
  ...
  const onSubmit = form.handleSubmit(async data => {
    ...
    for (const [inventory_item_id, item] of Object.entries(data.inventory_items)) {
      for (const [location_id, level] of Object.entries(item.locations)) {
        if (level.id) {
          const wasChecked =
            initialValues.current?.inventory_items?.[inventory_item_id]?.locations?.[location_id]
              ?.checked;
    ...
    inventory_items: items.reduce(
      (acc, item) => {
        const locationsMap = locations.reduce((locationAcc, location) => {
          const level = item.location_levels?.find(level => level.location_id === location.id);
