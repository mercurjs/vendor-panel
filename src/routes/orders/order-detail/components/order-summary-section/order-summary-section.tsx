...
import { AdminOrder, AdminOrderLineItem, AdminReservation, PaymentStatus } from '@medusajs/types';
import {
  Button,
  clx,
  Container,
...
import { useOrderCommission } from '../../../../../hooks/api/orders';
...
import { ExtendedAdminOrder, ExtendedAdminPaymentCollection } from '../../../../../types/order';
...
export const OrderSummarySection = ({
  ...
  const receivableReturns = useMemo(() => order.returns?.filter(r => !r.canceled_at), [order]);
  ...
  const showAllocateButton = useMemo(() => {
    ...
    return order.items.some(item => {
      const hasUnfulfilledQuantity =
        item.quantity && item.detail && item.quantity - item.detail.fulfilled_quantity > 0;

      return (
        item.variant?.manage_inventory && hasUnfulfilledQuantity && !reservationsMap.has(item.id)
      );
    });
  ...
  const handleMarkAsPaid = async (paymentCollection: Partial<ExtendedAdminPaymentCollection>) => {
...
const Item = ({
  ...
  const original_price =
    item.variant?.prices?.find(price => price.currency_code === currencyCode)?.amount || 0;
          ...
          <div className="flex items-center justify-end gap-x-4">
            <Text size="small">
              {original_price !== price && (
                <span className="mr-1 text-xs text-ui-fg-muted line-through">
...
const CostBreakdown = ({ order }: { order: ExtendedAdminOrder }) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="flex flex-col gap-y-2 px-6 py-4 text-ui-fg-subtle">
      <Cost
        label={t(automaticTaxesOn ? 'orders.summary.itemTotal' : 'orders.summary.itemSubtotal')}
        ...
        <div className="flex justify-between">
          ...
          >
            <span className="txt-small select-none">
              {t(automaticTaxesOn ? 'orders.summary.taxTotalIncl' : 'orders.summary.taxTotal')}
            </span>
            {hasTaxLines && (
              <TriangleDownMini
                style={{
          ...
          <div className="flex flex-col gap-1 pl-5">
            {Object.entries(taxCodes).map(([code, total]) => {
              return (
                ...
                >
                  <div>
                    <span className="txt-small font-medium text-ui-fg-subtle">{code}</span>
                  </div>
                  <div className="relative flex-1">
                    <div className="bottom-[calc(50% - 2px)] absolute h-[1px] w-full border-b border-dashed" />
                  </div>
