...
const CustomerGroupDisplay = ({
  ...
  const customerGroupIds =
    priceList.price_list_rules.find(rule => rule.attribute === 'customer.groups.id')?.value ||
    ([] as string[]);
  ...
  const filteredCustomerGroups = flatCustomerGroups.filter(group =>
