...
export const CategoryCombobox = forwardRef<HTMLInputElement, CategoryComboboxProps>(
  ({ value, onChange, className, onFocus, onBlur, ...props }, ref) => {
    ...
    const { product_categories, isPending, isError, error } = useProductCategories(queryParams, {
      enabled: open
    });
    ...
    const getAllCategoriesRecursive = useCallback(
      (categories: AdminProductCategoryResponse['product_category'][]): ProductCategoryOption[] => {
    ...
    return (
      ...
      >
        ...
        <RadixPopover.Content
          ...
          role="listbox"
          className={clx(
            '-left-2 z-50 w-[var(--radix-popper-anchor-width)] rounded-[8px] bg-ui-bg-base shadow-elevation-flyout',
            ...
            <Fragment>
              <div className="p-1">
                <button
                  ...
                  role="button"
                  className={clx(
                    'grid w-full appearance-none grid-cols-[20px_1fr] items-center justify-center gap-2 rounded-md px-2 py-1.5 text-left outline-none transition-fg',
            ...
            {options.length > 0 &&
              ...
              options.map((option, index) => (
                <div
                  key={option.value}
                  className={clx(
                    'grid cursor-pointer grid-cols-1 items-center gap-2 overflow-hidden bg-ui-bg-base transition-fg',
                    {
                ...
                >
                  ...
                  {option.has_children && !searchValue && (
                    <button
                      className={clx(
                        'flex size-8 appearance-none items-center justify-center rounded-md text-ui-fg-muted outline-none',
