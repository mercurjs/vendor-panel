...
export const CategorySelect = forwardRef<HTMLInputElement, CategoryComboboxProps>(
              ...
              {options.length > 0 &&
                ...
                options.map((option, index) => (
                  ...
                  >
                    <button
                      data-active={
                        showLevelUp ? focusedIndex === index + 1 : focusedIndex === index
                      }
                      type="button"
                      role="option"
                      className={clx(
...
const SelectItem = ({ value, children }: { value: string; children: React.ReactNode }) => {
  return (
    <Select.Item
      value={value}
      className={clx(
        'flex cursor-pointer items-center gap-2 overflow-hidden bg-ui-bg-base transition-fg hover:bg-ui-bg-field-hover'
      )}
    >
