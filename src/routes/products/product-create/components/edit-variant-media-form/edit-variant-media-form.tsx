...
// Get default values - variant-specific media only
const getDefaultValues = (initialMedia?: any[]) => {
    return { media: initialMedia || [] }
}
...
export const EditVariantMediaForm = ({ variantIndex, onSubmit, onCancel, onSaveMedia, initialMedia = [] }: EditVariantMediaFormProps) => {
    const [selection, setSelection] = useState<Record<string, true>>({})
    const { t } = useTranslation()

    const form = useForm<EditVariantMediaSchemaType>({
        defaultValues: getDefaultValues(initialMedia),
    ...
    // Submit handler - saves variant-specific media to parent form
    const handleSubmit = form.handleSubmit(async ({ media }) => {
        if (onSaveMedia) {
            // Save all media items in the form - these are variant-specific media
