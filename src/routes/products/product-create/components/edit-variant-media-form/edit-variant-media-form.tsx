...
export const EditVariantMediaForm = ({
  ...
  const [checkedUrls, setCheckedUrls] = useState<Record<string, true>>({});

  const availableMedia = useMemo(
    () => productMedia.filter((m): m is MediaItem & { url: string } => !!m.url),
    [productMedia]
  );
  ...
  const handleSave = useCallback(() => {
    ...
    onClose?.();
  }, [getEffectiveThumbnailUrl, selectedUrls, onSaveMedia, variantIndex, onClose]);
    ...
    <div className="flex size-full flex-col overflow-hidden">
      ...
      <CommandBar open={!!checkedCount}>
        <CommandBar.Bar>
          ...
          <CommandBar.Command
            action={() => {
              Object.keys(checkedUrls).forEach(url => handleDeselectImage(url));
              setCheckedUrls({});
            }}
    ...
    >
      ...
      >
        <Checkbox
          ...
          data-testid={`variant-media-check-${image.url}`}
        />
      </div>
      <img
        src={image.url}
...
const UnselectedImageCard = ({ image, onSelect }: UnselectedImageCardProps) => {
  ...
  return (
    <button
      ...
      data-testid={`variant-media-select-${image.url}`}
    >
      <img
        src={image.url}
        alt=""
