...
import { ProductCreateAttributesForm, ProductCreateAttributesFormRef } from "../product-create-attributes-form/product-create-attributes-form"
...
export const ProductCreateForm = ({
  ...
  const dynamicStockLocationSchema = useMemo(() => {
    ...
    stock_locations?.forEach((location: any) => {
      stockLocationFields[`stock_locations.${location.id}`] = z.object({
  ...
  const handleSubmit = form.handleSubmit(async (values, e) => {
    ...
    if (allAttributes && allAttributes.length > 0) {
      ...
      const knownFields = [
        'title', 'subtitle', 'handle', 'description', 'discountable', 'type_id',
        'collection_id', 'shipping_profile_id', 'categories', 'secondary_categories',
        'tags', 'sales_channels', 'origin_country', 'material', 'width', 'length',
        'height', 'weight', 'mid_code', 'hs_code', 'options', 'enable_variants',
        'variants', 'media', 'regionsCurrencyMap', 'status', 'additional_data'
      ]

      // Find dynamic attribute fields (those not in known fields)
      dynamicAttributeFields = allFieldNames.filter(fieldName =>
        !knownFields.includes(fieldName) &&
        !fieldName.endsWith('UseForVariants') // Exclude "Use for Variants" fields
      )
    }
    ...
    allAttributes?.forEach((attr: any) => {
      if (attr.ui_component === 'multivalue') {
        ...
        if (isUsedForVariants && selectedValueIds && Array.isArray(selectedValueIds) && selectedValueIds.length > 0) {
          const selectedValues = selectedValueIds
            .map((valueId: string) => {
              const possibleValue = (attr as any).possible_values?.find((pv: any) => pv.id === valueId)
    ...
    dynamicAttributeFields.forEach(fieldName => {
      ...
      if (Array.isArray(value) && value.length > 0) {
        value.forEach((valueId: string) => {
          const possibleValue = (attribute as any).possible_values?.find((pv: any) => pv.id === valueId)
      ...
      // For single values, convert value ID to actual value
        ...
        if ((attribute as any).possible_values && typeof value === 'string') {
          const possibleValue = (attribute as any).possible_values.find((pv: any) => pv.id === value)
          if (possibleValue) {
    ...
    const mappedVariants = (finalPayload as any).variants.map((variant: any) => {
      ...
      variantAttributes.forEach(attr => {
        ...
        const matchingValue = attrValues.find(value =>
    ...
    const finalPayloadWithAdditionalData = {
      ...
      additional_data: additionalDataValues.length > 0 ? {
    ...
    if (productData?.product?.variants && stock_locations.length > 0) {
      try {
        for (const variant of productData.product.variants) {
          if (variant.inventory_items && variant.inventory_items.length > 0) {
            ...
            if (variantStockLocations) {
              ...
              Object.entries(variantStockLocations).forEach(([locationId, locationData]: [string, any]) => {
                if (locationData.checked && locationData.quantity !== "" && locationData.quantity !== 0) {
        ...
        toast.error("Product created successfully, but there was an issue assigning stock locations.")
      }
    }
          ...
          >
            ...
            <RouteFocusModal.Body className="size-full overflow-hidden">
              ...
              <ProgressTabs.Content
                ...
                value={Tab.ATTRIBUTES}
              >
                <ProductCreateAttributesForm form={form as any} ref={attributesFormRef} />
              </ProgressTabs.Content>
              <ProgressTabs.Content
                className="size-full overflow-y-auto"
                value={Tab.VARIANTS}
