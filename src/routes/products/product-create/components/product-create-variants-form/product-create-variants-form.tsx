...
export const ProductCreateVariantsForm = ({
  ...
  const variantData = useMemo(() => {
    ...
    if (variantAttributes.length > 0) {
      const totalCombinations = variantAttributes.reduce((acc, attr) => acc * attr.selectedValues.length, 1)
  ...
  const variantStructureKey = useMemo(() => {
    return variantAttributes.map(attr => `${attr.handle}:${attr.selectedValues.map(v => v.id).join(',')}`).join('|')
  }, [variantAttributes])
  ...
  return (
    <div className="flex flex-col justify-between h-full divide-y border-ui-border">
      <DataGrid
        columns={columns}
  ...
  return useMemo(
    () => [
      ...
      columnHelper.column({
        ...
        cell: (context) => {
          const rowData = context.row.original as any
          return <DataGrid.ReadonlyCell context={context}>{rowData.autoGeneratedTitle}</DataGrid.ReadonlyCell>
        },
        disableHiding: true,
        pin: 'left',
      }),
