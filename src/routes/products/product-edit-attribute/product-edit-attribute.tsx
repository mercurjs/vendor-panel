...
export const ProductEditAttribute = () => {
  ...
  const attribute = product?.informational_attributes?.find(a => a.attribute_id === attribute_id);

  const attributeDefinition = attributes?.find(a => a.id === attribute_id);

  if (!isPending && !isFetching && !attribute) {
    throw json({ message: `An attribute with ID ${attribute_id} was not found` }, 404);
  }
  ...
  return (
    <RouteDrawer>
      <RouteDrawer.Header>
        <Heading>{t('products.edit.attributes.editHeader')}</Heading>
      </RouteDrawer.Header>
      {attribute && (
