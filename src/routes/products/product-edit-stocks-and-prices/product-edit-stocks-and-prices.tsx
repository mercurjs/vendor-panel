...
export const ProductEditStocksAndPrices = () => {
  ...
  const refetchAll = async () => {
    await Promise.all([refetchProduct(), refetchStockLocations(), refetchInventory()]);
  };
  ...
  const isRefetching = isProductRefetching || isStockLocationsRefetching || isInventoryRefetching;
  const ready =
    !isPending && !!product && !!inventoryItemsWithLevels && !!stock_locations && !isRefetching;
