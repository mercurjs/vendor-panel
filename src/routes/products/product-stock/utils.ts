...
import { ExtendedAdminProductVariant, ProductStockGridRow } from '../../../types/products';

export function isProductVariant(row: ProductStockGridRow): row is ExtendedAdminProductVariant {
...
export function isProductVariantWithInventoryPivot(
  ...
  return (
    'inventory_items' in row && Array.isArray(row.inventory_items) && row.inventory_items.length > 0
  );
}

export function getDisabledInventoryRows(variants: ExtendedAdminProductVariant[]) {
