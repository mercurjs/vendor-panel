      ...
      {watchCampaignChoice === 'existing' && (
        <Form.Field
          ...
          render={({ field: { onChange, ref, ...field } }) => {
            return (
              <Form.Item>
                <Form.Label>{t('promotions.form.campaign.existing.title')}</Form.Label>
                      ...
                      {!campaigns.length && (
                        <div className="flex h-[120px] flex-col items-center justify-center gap-2 p-2">
                          <span className="txt-small font-medium text-ui-fg-subtle">
                            {t('promotions.form.campaign.existing.placeholder.title')}
                          </span>
                          <span className="txt-small text-ui-fg-muted">
                            {t('promotions.form.campaign.existing.placeholder.desc')}
                          </span>
                        </div>
                      )}
                      {campaigns.map(c => (
...
export const AddCampaignPromotionForm = ({ promotion, campaigns }: EditPromotionFormProps) => {
  const { t } = useTranslation();
