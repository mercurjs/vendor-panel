...
export const PromotionGeneralSection = ({ promotion }: PromotionGeneralSectionProps) => {
  ...
  return (
    <Container className="divide-y p-0">
      ...
      {(promotion as any).metadata?.title && (
        <div className="grid grid-cols-2 items-start px-6 py-4 text-ui-fg-subtle">
          <Text
            size="small"
            weight="plus"
          ...
          <Text
            ...
            className="text-pretty"
          >
            {(promotion as any).metadata.title as string}
          </Text>
        </div>
      )}

      {(promotion as any).metadata?.description && (
        <div className="grid grid-cols-2 items-start px-6 py-4 text-ui-fg-subtle">
          <Text
            size="small"
            weight="plus"
          ...
          >
            {(promotion as any).metadata.description as string}
          </Text>
        </div>
      )}

      {(promotion as any).metadata?.conditions && (
        <div className="grid grid-cols-2 items-start px-6 py-4 text-ui-fg-subtle">
          <Text
            size="small"
            weight="plus"
          ...
          <Text
            ...
            className="whitespace-pre-wrap text-pretty"
          >
            {(promotion as any).metadata.conditions as string}
          </Text>
        </div>
      )}

      {(promotion as any).metadata?.min_purchase && (
        <div className="grid grid-cols-2 items-start px-6 py-4 text-ui-fg-subtle">
          <Text
            size="small"
            weight="plus"
          ...
          <Text
            ...
            className="text-pretty"
          >
            {(promotion as any).metadata.min_purchase as string}
          </Text>
        </div>
      )}
    </Container>
