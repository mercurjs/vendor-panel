...
import { useOrderReturnRequest, useUpdateOrderReturnRequest } from '../../../hooks/api/requests';
...
export function RequestOrderReturn() {
  ...
  const { stock_locations, isLoading: isStockLocationsLoading } = useStockLocations();
      ...
      <RouteDrawer.Body>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleUpdateOrderReturnRequest)}>
            ...
            />
            {form.watch('status') === 'refunded' && (stock_locations || []).length > 0 && (
              <Form.Field
                control={form.control}
                name="location_id"
                render={({ field: { onChange, value, ...field } }) => (
