    ...
    await mutateAsync(
      {
        supported_currencies:
          store.supported_currencies?.filter(c => !ids.includes(c.currency_code)) ?? []
      },
      {
        onSuccess: () => {
          setRowSelection({});
...
const CurrencyActions = ({
  ...
  const handleRemove = async () => {
    ...
    await mutateAsync(
      {
        supported_currencies: supportedCurrencies.filter(c => c.currency_code !== currency.code)
      },
      {
        onSuccess: () => {
          toast.success(t('store.toast.currenciesRemoved'));
    ...
    await mutateAsync(
      {
        supported_currencies: supportedCurrencies.map(c => {
          ...
          return {
            ...c,
            is_tax_inclusive:
              c.currency_code === currency.code ? !pref?.is_tax_inclusive : undefined
          };
        })
      },
      {
...
const columnHelper = createColumnHelper<HttpTypes.AdminCurrency & { is_tax_inclusive?: boolean }>();
  ...
  return useMemo(
    () => [
      ...
      columnHelper.display({
        id: 'actions',
        cell: ({ row, table }) => {
          const { supportedCurrencies, storeId, defaultCurrencyCode, preferencesMap } = table
            .options.meta as {
            supportedCurrencies: HttpTypes.AdminStoreCurrency[];
            storeId: string;
            defaultCurrencyCode: string;
