...
export const filterCustomers = (
  ...
  return customers.filter(customer => {
    ...
    if (filters.groups?.length) {
      const hasMatchingGroup = customer.groups?.some(group => filters.groups?.includes(group.id));
...
const compareStrings = (a: string | null, b: string | null, desc = false): number => {
...
export const sortCustomers = (customers: AdminCustomer[], order?: string): AdminCustomer[] => {
